...
char nextStep(char player)
{
    ...
    gameScr();
    printf("%10s", " ");
    printf("It's now turn for %c player\n\n", player);
    printf("%10s", " ");
    printf("Give the row number [1-3]: ");
    scanf("%u", &row);
    fflush(stdin);
    printf("%10s", " ");
    printf("Give the col number [1-3]: ");
    scanf("%u", &col);
    fflush(stdin);
    ...
    if (TicTacToe[row - 1][col - 1] == NOTHING)
    {
        ...
        if (checkWin(player) == WIN)
            return WIN; //! Return win, if win condition is approached.
        if (--feld == 0)
            return GAMEOVER;          //! If all felds are full, game is draw.
